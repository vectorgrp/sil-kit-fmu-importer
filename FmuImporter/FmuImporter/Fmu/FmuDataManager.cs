...
public class FmuDataManager
{
  ...
  public Dictionary<VariableTypes, List<ConfiguredVariable>> OutputConfiguredClockedVariables { get; }
  public Dictionary<long, ConfiguredStructure> OutputConfiguredClockedStructures { get; }
  ...
  private Dictionary<uint, uint[]?> OutpoutStructClockRefs { get; }
    ...
    bool useClockPubSubElements)
  {
    ...
    OutputConfiguredClockedVariables = new Dictionary<VariableTypes, List<ConfiguredVariable>>();
    ...
    OutpoutStructClockRefs = new Dictionary<uint, uint[]?>();
  }
  ...
  private void AddConfiguredVariable(ConfiguredVariable c)
  {
    switch (c.FmuVariableDefinition.Causality)
    {
      case Variable.Causalities.Output:
        // clocks
        if (c.FmuVariableDefinition.VariableType == VariableTypes.TriggeredClock)
        {
          OutputConfiguredClocks.Add(c);
          break;
        }
        ...
        {
          if (OutputConfiguredClockedVariables.TryGetValue(c.FmuVariableDefinition.VariableType, out var refClockedValue))
          {
            refClockedValue.Add(c);
          }
          else
          {
            OutputConfiguredClockedVariables[c.FmuVariableDefinition.VariableType] = [c];
          }
          break;
        }
    ...
    OptionalType structType)
  {
    ...
    var isClocked = configuredVariable.FmuVariableDefinition.VariableType == VariableTypes.TriggeredClock ||
                    (configuredVariable.FmuVariableDefinition.Clocks != null && 
                     configuredVariable.FmuVariableDefinition.Clocks.Length > 0);

    switch (configuredVariable.FmuVariableDefinition.Causality)
    {
      case Variable.Causalities.Output:
        // add clock refs of structure members to dictionary
        if (configuredVariable.FmuVariableDefinition.VariableType == VariableTypes.TriggeredClock)
        {
          OutpoutStructClockRefs[configuredVariable.FmuVariableDefinition.ValueReference] = configuredVariable.FmuVariableDefinition.Clocks;
        }

        // check if this structure already exists in clocked structures
        if (_outputConfiguredClockedStructureByName.ContainsKey(structName) || isClocked)
        {
          // move existing structure from non-clocked to clocked if necessary
          if (_outputConfiguredStructureByName.TryGetValue(structName, out var existingStruct))
          {
            _outputConfiguredClockedStructureByName[structName] = existingStruct;
            OutputConfiguredClockedStructures[existingStruct.StructureId] = existingStruct;
            _outputConfiguredStructureByName.Remove(structName);
            OutputConfiguredStructures.Remove(existingStruct.StructureId);
          }
          targetStructureInternal = _outputConfiguredClockedStructureByName;
          targetStructure = OutputConfiguredClockedStructures;
        }
        else
        {
      ...
      case Variable.Causalities.Input:
        // check if this structure already exists in clocked structures
        if (_inputConfiguredClockedStructureByName.ContainsKey(structName) || isClocked)
        {
          // move existing structure from non-clocked to clocked if necessary
          if (_inputConfiguredStructureByName.TryGetValue(structName, out var existingStruct))
          {
            _inputConfiguredClockedStructureByName[structName] = existingStruct;
            InputConfiguredClockedStructures[existingStruct.StructureId] = existingStruct;
            _inputConfiguredStructureByName.Remove(structName);
            InputConfiguredStructures.Remove(existingStruct.StructureId);
          }
          targetStructureInternal = _inputConfiguredClockedStructureByName;
          targetStructure = InputConfiguredClockedStructures;
        }
        else
        {
  ...
  private void ValidateCommInterfaceMapping()
  {
    var targetStructureDictionaryValues = new List<Dictionary<long, ConfiguredStructure>.ValueCollection>()
    {
      ...
      InputConfiguredClockedStructures.Values,
      OutputConfiguredStructures.Values,
      OutputConfiguredClockedStructures.Values
    };
  ...
  public List<Tuple<long, byte[]>> GetClocks(HashSet<long> activeInputClocks) // a input clock can also be clocked
  {
    var returnData = new List<Tuple<long, byte[]>>();

    // keep clocks which are not clocked, or with active clocks
    var filteredClocks = OutputConfiguredClocks
      .Where(cv => cv.FmuVariableDefinition.Clocks == null ||
                    cv.FmuVariableDefinition.Clocks.Length == 0 ||
                    cv.FmuVariableDefinition.Clocks.Any(c => activeInputClocks.Contains(c)))
      .ToList();

    if (filteredClocks.Count == 0)
    {
      return returnData;
    }

    var clockRefQuery = filteredClocks.Select(cv => cv.FmuVariableDefinition.ValueReference).ToArray();
    Binding.GetValue(clockRefQuery, out var clockResult, VariableTypes.TriggeredClock);

    foreach (var clockResVar in clockResult.ResultArray)
    {
      var configuredVar = OutputConfiguredClocks.Find(x => x.FmuVariableDefinition.ValueReference == clockResVar.ValueReference);
      ...
      var byteArray = dc.TransformFmuToSilKitData(clockResVar, configuredVar!);
  ...
  public List<Tuple<long, byte[]>> GetClockedVariableData(HashSet<long> activeInputClocks, HashSet<long> activeOutputClocks)
  {
    var returnData = new List<Tuple<long, byte[]>>();

    if (activeInputClocks.Count == 0 && activeOutputClocks.Count == 0)
    {
      // if no active clocks, do not retrieve any clocked variable data
      return returnData;
    }

    foreach (var typeVars in OutputConfiguredClockedVariables)
    {
      // include only variables without clocks, or those with active clocks
      var filteredVariables = typeVars.Value
        .Where(cv => cv.FmuVariableDefinition.Clocks!.Any(c => (activeInputClocks.Contains(c) || activeOutputClocks.Contains(c))))
        .ToList();
      ...
      var valueRefQuery = from configuredVariable in filteredVariables
      ...
      HandleOutputResultData(result, typeVars, ref returnData);
    }
  ...
  static private void HandleOutputResultData(ReturnVariable result,
                                             KeyValuePair<VariableTypes, List<ConfiguredVariable>> typeVars,
                                             ref List<Tuple<long, byte[]>> returnData)
  {
  ...
  public List<Tuple<long, byte[]>> GetClockedStructureData(HashSet<long> activeInputClocks)
  {
    var returnData = new List<Tuple<long, byte[]>>();

    // track active clock refs (in and out)
    HashSet<long>? activeClockRefs = new HashSet<long>();

    // handle input clocks
    if (activeInputClocks.Count > 0)
    {
      foreach (var inputClockRef in activeInputClocks)
      {
        activeClockRefs.Add(inputClockRef);
      }
    }

    // filter output clocks to get
    var outputClocksToGet = new List<uint>();
    foreach (var clockref in OutpoutStructClockRefs)
    {
      if (clockref.Value == null || clockref.Value.Any(c => activeClockRefs.Contains(c)))
      {
        outputClocksToGet.Add(clockref.Key);
      }
    }

    // store output clock variable data for serialization
    var outputClockData = new Dictionary<long, ReturnVariable.Variable>();

    if (outputClocksToGet.Count != 0)
    {
      Binding.GetValue(outputClocksToGet.ToArray(), out var clockResult, VariableTypes.TriggeredClock);

      foreach (var clockVar in clockResult.ResultArray)
      {
        // get active clocks
        if (clockVar.Values.Length > 0 && clockVar.Values[0] is bool clockValue && clockValue)
        {
          activeClockRefs.Add(clockVar.ValueReference);
          outputClockData[clockVar.ValueReference] = clockVar;
        }
      }
    }

    // retrieve individual structures
    foreach (var configuredStructure in OutputConfiguredClockedStructures.Values)
    {
      ...
      foreach (var structureMember in configuredStructure.StructureMembers)
      {
        ...
        // skip if clocked var with clock not active
        if (structureMember.FmuVariableDefinition.Clocks != null &&
            structureMember.FmuVariableDefinition.Clocks.Length > 0 &&
            !structureMember.FmuVariableDefinition.Clocks.Any(c => activeClockRefs.Contains(c)))
        {
          continue;
        }

        // handle TriggeredClock variables
        if (structureMember.FmuVariableDefinition.VariableType == VariableTypes.TriggeredClock)
        {
          if (outputClockData.Count > 0 &&
              outputClockData.TryGetValue(structureMember.FmuVariableDefinition.ValueReference, out var clock))
          {
            memberData.Add(clock);
          }
          // skip clock variables that are not active output clocks
          continue;
        }
  ...
  public void SetStructure(long structureId, byte[] silKitData)
  {
    ...
    if (!success)
    {
      // also check clocked structures
      success = InputConfiguredClockedStructures.TryGetValue(structureId, out configuredStructure);
    }
    ...
    out List<List<int>> binSizes)
  {
    ...
    {
      // also check clocked structures
      success = InputConfiguredClockedStructures.TryGetValue(refValue, out configuredStructure);
    }
