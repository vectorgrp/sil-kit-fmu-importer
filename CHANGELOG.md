...
* Support of the CAN bus part of the FMI Layered Standard. (Classical CAN only)
  * The FMU importer now supports Event Mode.
  * The FMU importer now supports triggered clocks and clocked variables.
...
* Updated SIL Kit to version 4.0.56
* Quality of life features:
  * SIL Kit configuration file argument defaults to `<path of executable>/Config.silkit.yaml`
  * Communication Interface Description file defaults to the one that would be generated by the CommInterfaceExporter.
  * Added variables names or valueRefs to error messages when a case happens to be linked to a variable.

### Fixed

* Linear transformations on non-scalar variable are now applied to all elements, not just the first one.
* Grouped retrieving variable values by type when possible.
* Discard results on various FMI calls are now handled correctly.
* Crash with some invalid FMUs (e.g. missing `ModelDescription.xml`).
